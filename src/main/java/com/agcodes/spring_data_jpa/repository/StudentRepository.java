...
public interface StudentRepository
  ...
  @Query(value = "SELECT * FROM repository WHERE first_name =?1 and age=?2",nativeQuery = true)
  List<Student> selectStudentWhereFirstNameAndAgeEqualsNative(String firstName,Integer age);
  @Query(value = "SELECT * FROM repository WHERE first_name =:firstName and age>:age",nativeQuery = true)
